...
import { 
  ...
  Plus, ChevronDown, Wrench, ShieldCheck, Briefcase, Car
} from 'lucide-react';
...
export const MobileApp = ({ 
  ...
  const [mobilePortalMode, setMobilePortalMode] = useState('customer');
    ...
    return (
      <div className="min-h-screen bg-[#07090e] text-slate-100 flex justify-center selection:bg-emerald-500 selection:text-slate-950 font-sans">
        <div className="w-full max-w-md min-h-screen bg-[#090d16] border-x border-white/5 relative flex flex-col shadow-2xl">
          ...
          {/* Mobile Top Bar to Switch Back to Customer */}
          <div className="sticky top-0 z-50 bg-[#0a0f1d] border-b border-emerald-500/20 px-4 py-2 flex items-center justify-between text-xs">
            <span className="font-black text-emerald-400 flex items-center gap-1">
              <Briefcase className="w-4 h-4" />
              <span>기사 파트너 전용</span>
            </span>
            <button
              onClick={() => setMobilePortalMode('customer')}
              className="px-2.5 py-1 rounded-xl bg-slate-800 hover:bg-slate-700 text-cyan-300 font-bold border border-cyan-500/30 flex items-center gap-1 transition-colors text-[11px]"
            >
          ...
          <PartnerPortal
            ...
            onSwitchToCustomer={() => setMobilePortalMode('customer')}
          />
        </div>
      </div>
    );
      ...
      <div className="w-full max-w-md min-h-screen bg-[#090d16] border-x border-white/5 relative flex flex-col shadow-2xl pb-16">
        ...
        <header className="sticky top-0 z-40 bg-[#090d16]/95 backdrop-blur-md px-4 py-3 border-b border-white/10 flex items-center justify-between">
          ...
          <button
            ...
            className="flex items-center gap-1.5 text-xs font-black text-white hover:text-cyan-400 transition-colors py-1 px-2.5 rounded-xl bg-slate-900/80 border border-white/5"
          >
          ...
          {/* Right: Switch to Partner Portal on Mobile */}
          <button
            onClick={() => setMobilePortalMode('partner')}
            className="px-2.5 py-1.5 rounded-xl bg-emerald-950/70 hover:bg-emerald-900/70 text-emerald-300 text-[11px] font-bold border border-emerald-500/30 flex items-center gap-1 transition-colors"
            title="기사 파트너 모드로 전환"
          >
        ...
        <main className="flex-grow">
          ...
          {mobileTab === 'mypage' && (
            <MobileMyPageTab
              ...
              onSwitchToPartner={() => setMobilePortalMode('partner')}
            />
          )}
        </main>
